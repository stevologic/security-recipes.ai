...
def request_from_args(args: argparse.Namespace, ledger: dict[str, Any]) -> dict[str, Any]:
    if args.request:
    ...
    if args.use_ledger_evidence:
        workflow = workflows_by_id(ledger).get(str(payload.get("workflow_id") or ""))
        if workflow is None:
            raise SecureContextLineageDecisionError(
                "--use-ledger-evidence requires a registered --workflow-id"
            )
        if not payload.get("source_ids"):
            payload["source_ids"] = [str(item) for item in as_list(workflow.get("source_ids")) if item]
        if not payload.get("source_hashes"):
            raw_hashes = workflow.get("source_hashes")
            registered_hashes = raw_hashes if isinstance(raw_hashes, dict) else {}
            payload["source_hashes"] = [
                str(registered_hashes[source_id])
                for source_id in payload["source_ids"]
                if source_id in registered_hashes and registered_hashes[source_id]
            ]
        if not payload.get("context_package_hash"):
            payload["context_package_hash"] = workflow.get("context_package_hash")
    for flag in [
        "contains_secret",
        "context_hash_mismatch",
        "identity_used_after_revocation",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-ledger-evidence",
        action="store_true",
        help="Fill omitted source hashes and context package hash from the registered workflow envelope.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        request = request_from_args(args, ledger)
