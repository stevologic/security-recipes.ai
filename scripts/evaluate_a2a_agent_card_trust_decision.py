...
def sample_card_by_id(pack: dict[str, Any], sample_card_id: str) -> dict[str, Any]:
    for sample in as_list(pack.get("sample_agent_cards")):
        if isinstance(sample, dict) and str(sample.get("id") or "") == sample_card_id:
            card = sample.get("agent_card")
            if not isinstance(card, dict):
                raise A2AAgentCardTrustError(
                    f"sample Agent Card {sample_card_id!r} does not contain an agent_card object"
                )
            return sample
    raise A2AAgentCardTrustError(f"sample Agent Card {sample_card_id!r} is not registered in the pack")
...
def parse_args() -> argparse.Namespace:
    ...
    card_input = parser.add_mutually_exclusive_group(required=True)
    card_input.add_argument("--agent-card", type=Path)
    card_input.add_argument(
        "--sample-card-id",
        help="Evaluate a named sample_agent_cards entry from the selected pack.",
    )
...
def main() -> int:
    ...
    try:
        ...
        sample = sample_card_by_id(pack, args.sample_card_id) if args.sample_card_id else None
        card = as_dict(sample.get("agent_card")) if sample else load_json(args.agent_card)
        profile_id = args.profile_id or (str(sample.get("expected_profile_id") or "") if sample else "")
        if not profile_id:
            raise A2AAgentCardTrustError("--profile-id is required unless the selected sample declares expected_profile_id")
        request = {
            "agent_card": card,
            "approved_skill_ids": args.approved_skill_id,
            "correlation_id": args.correlation_id,
            "declared_controls": args.declared_control or (as_list(sample.get("declared_controls")) if sample else []),
            "expected_domain": args.expected_domain,
            "production": args.production or (bool(sample.get("production")) if sample else False),
