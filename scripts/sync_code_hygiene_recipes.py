...
RECIPE_DESCRIPTION_SUFFIX = (
    " Includes scoped detection, verification, and stop conditions."
)
FAMILY_DESCRIPTION_SUFFIX = (
    " Choose bounded workflows with detection, tests, and stop conditions."
)
...
def recipe_meta_description(record: dict[str, Any]) -> str:
    """Build page-specific search copy without shared opening boilerplate."""

    title = str(record["title"]).strip().rstrip(".")
    goal = str(record["goal"]).strip().rstrip(".")
    base = f"{title}: {goal[:1].upper()}{goal[1:]}."
    candidate = f"{base}{RECIPE_DESCRIPTION_SUFFIX}"
    if len(candidate) <= META_DESCRIPTION_LIMIT:
        return candidate
    if len(base) <= META_DESCRIPTION_LIMIT:
        return base
    raise ValueError(
        f"code-hygiene search description exceeds {META_DESCRIPTION_LIMIT} characters: "
        f"{record.get('family')}/{record.get('slug')}"
    )


def family_meta_description(family: dict[str, Any]) -> str:
    """Expand terse ecosystem labels into complete collection descriptions."""

    base = str(family["description"]).strip().rstrip(".") + "."
    candidate = f"{base}{FAMILY_DESCRIPTION_SUFFIX}"
    if len(candidate) > META_DESCRIPTION_LIMIT:
        raise ValueError(
            f"code-hygiene family description exceeds {META_DESCRIPTION_LIMIT} characters: "
            f"{family.get('title')}"
        )
    return candidate
...
def render_recipe(
    ...
    description = recipe_meta_description(record)
