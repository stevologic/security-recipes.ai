...
// incomparable rankings or moving 152k records onto the main thread.
...
function readCveStats() {
  try {
    ...
    return { total: 0, critical: 0, high: 0, kev: 0, stableOverrides: 0 };
  }
}
...
module.exports = function recipeBrowser() {
  ...
  return (
    ...
    `<p>Choose reviewed workflow guidance for a security task, or search the complete high and critical CVE catalog. Each collection keeps the ranking, filters, and retrieval path that fit its scale.</p>` +
    ...
    `<div><dt>High / critical CVEs</dt><dd>${count(cve.total)}</dd></div>` +
    ...
    `<span>CVE catalog</span><strong>${count(cve.total)}</strong><small>${count(cve.critical)} critical · ${count(cve.high)} high · ${count(cve.kev)} KEV</small></button>` +
    `</div>` +
    `<noscript><p class="recipe-library__noscript">JavaScript is required for in-page filtering. ` +
    `<a href="/prompt-library/cve/">Open the complete CVE catalog</a>, or browse the first curated results below.</p></noscript>` +
    ...
    `<h3>Search 10 years of high and critical CVEs</h3>` +
    `<p>Exact identifiers fetch one deterministic shard. Title and filter searches use a compressed worker index without blocking the page.</p></div>` +
    ...
    `<a href="/prompt-library/cve/">Catalog methodology</a>` +
    `</div></div>` +
