...
module.exports = function recipeBrowser() {
  ...
  return (
    ...
    `data-recipe-mcp="/mcp" data-cve-workflow-index="/api/cve-workflows.json" ` +
    `data-recipe-total="${stats.total}" data-cve-total="${cve.total}" ` +
    ...
    `<span class="recipe-library__eyebrow">CVE intelligence + reviewed workflows</span>` +
    `<h2 id="recipe-library-heading">Find the vulnerability. Run the right workflow.</h2>` +
    `<p>Start with a CVE, affected product, security finding, or desired outcome. Source evidence and human-reviewed action paths stay connected, so teams can move from “what is this?” to a bounded, verifiable change.</p>` +
    `<form class="recipe-library__mission-search" data-library-search-form role="search">` +
    `<label for="recipe-library-search">Search the connected security library</label>` +
    `<div class="recipe-library__mission-field">` +
    `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m21 21-4.3-4.3M10.8 18a7.2 7.2 0 1 1 0-14.4 7.2 7.2 0 0 1 0 14.4Z"/></svg>` +
    `<input id="recipe-library-search" type="search" maxlength="160" autocomplete="off" spellcheck="false" ` +
    `data-library-search placeholder="CVE-2024-3400, Log4j, dependency upgrade, access control…">` +
    `<button type="submit">Search</button></div>` +
    `<p>Exact CVE IDs open the source record and matched workflows immediately. Other terms search the curated workflow collection.</p>` +
    `</form>` +
    `<ol class="recipe-library__journey" aria-label="From CVE evidence to verified remediation">` +
    `<li><span>01</span><strong>Find</strong><small>Identity, severity, exploitation, and affected surface</small></li>` +
    `<li><span>02</span><strong>Act</strong><small>Matched intake, audit, containment, and remediation workflows</small></li>` +
    `<li><span>03</span><strong>Prove</strong><small>Required evidence, verification, rollback, and stop conditions</small></li>` +
    `</ol>` +
    ...
    `<span>Curated workflows</span><strong>${count(stats.total)}</strong><small>Reviewed instructions, evidence checks, and bounded change paths</small></button>` +
    ...
    `<span>CVE intelligence</span><strong>${count(cve.total)}</strong><small>${count(cve.critical)} critical · ${count(cve.high)} high · ${count(cve.medium)} medium · ${count(cve.kev)} KEV</small></button>` +
    `</div>` +
    ...
    `<p>Narrow the reviewed collection by outcome. Workflows explicitly linked to CVE archetypes appear inside matching CVE records.</p>` +
    ...
    `<div><span class="recipe-library__eyebrow">Understand, then act</span>` +
    `<h3>Search 10 years of medium, high, and critical CVEs</h3>` +
    `<p>Paste a complete CVE ID to open the exact record, source facts, and matched curated workflows. Or search a product or vulnerability title, then narrow the full catalog by severity, year, or CISA KEV status.</p></div>` +
    ...
    `</div></div>` +
    `<div id="cve-results" data-cve-catalog data-cve-catalog-deferred ` +
    `data-cve-catalog-base="/api/cve-catalog/" data-cve-workflow-index="/api/cve-workflows.json"></div>` +
    `</section>` +
    `</section>`
  );
};
