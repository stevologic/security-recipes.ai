...
// Cap per category for the inline seed payload, so every lane has cards to
// show even if /recipes-browser.json never arrives.
const SEED_PER_CATEGORY = 48;

// Inline seed: the SSR first page plus a bounded slice of every non-CVE lane.
// The client filters over this until the full feed loads, so a category click
// can never land on an empty grid just because the fetch is slow or failed.
function seedCards(cards) {
  // Keyed by url, not slug: recipes in different tool directories can share a
  // filename-derived slug, but their page urls are always unique.
  const seen = new Set();
  const seed = [];
  const add = (card) => {
