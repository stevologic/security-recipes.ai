...
function browserStats(cards) {
  ...
  return { total: cards.length, cve, highImpact, zeroDay, categoryCounts };
}
...
function renderCardHtml(card) {
  ...
  return (
    ...
    `<div class="recipe-browser-card__facets" aria-label="Recipe facets">${card.facets.map((f) => `<span>${escapeHtml(f.replace(/-/g, " "))}</span>`).join("")}</div>` +
...
module.exports = { CATEGORIES, browserCardObjects, browserStats, renderCardHtml };
