...
//        keywords, cve, canonicalCveRoute, date, sourcePath, isHome, isSection }
function seoHead(ctx) {
  ...
  const canonicalCve = /^CVE-\d{4}-\d{4,}$/i.test(String(ctx.cve || "").trim())
    ? String(ctx.cve).trim().toUpperCase()
    : "";
        ...
        "@graph": [
          {
            ...
            potentialAction: {
              "@type": "SearchAction",
              target: `${site.baseURL.replace(/\/$/, "")}/recipes/?q={search_term_string}`,
  ...
  if (isPage) {
    ...
    const isCveDetail =
      ctx.url.startsWith("/recipes/cve/") && ctx.url !== "/recipes/cve/";
    if (isCveDetail) {
      ...
      const record = byUrl.get(ctx.url);
      crumbs.push({
        name: record?.title || pageTitle,
        item: absURL(record?.url || ctx.url),
      });
    } else {
      let acc = "";
