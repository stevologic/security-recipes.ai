...
// Search engines measure snippets in rendered pixels rather than characters,
// but these conservative ceilings keep the important subject visible before
// truncation on common desktop and mobile result layouts. The visible H1 and
// article body retain the complete editorial wording.
const MAX_DESCRIPTION_LENGTH = 165;
const MAX_TITLE_LENGTH = 70;
const MAX_BRANDED_TITLE_LENGTH = 65;
...
function seoTitle(pageTitle) {
  ...
  const suffix = ` | ${brand}`;
  const branded = `${title}${suffix}`;
  if (branded.length <= MAX_BRANDED_TITLE_LENGTH) return branded;
  // Google already presents the site name beside the result. When the full
  // brand would crowd out the page subject, omit it instead of throwing away
  // high-intent product, framework, agent, or vulnerability terms.
  return truncateWords(title, MAX_TITLE_LENGTH);
}
...
const CATALOG_FRESHNESS_SOURCE_PATHS = new Set([
  "_index.md",
  "cve-database/_index.md",
]);
let catalogUpdatedAtCache;

function resolvedCatalogUpdatedAt(ctx) {
  if (Object.hasOwn(ctx || {}, "catalogUpdatedAt")) {
    return validIso(ctx.catalogUpdatedAt);
  }
  if (catalogUpdatedAtCache !== undefined) {
    return catalogUpdatedAtCache;
  }
  try {
    const { loadCatalogUpdatedAt } = require("./cve-indexability");
    catalogUpdatedAtCache = validIso(loadCatalogUpdatedAt());
  } catch {
    catalogUpdatedAtCache = "";
  }
  return catalogUpdatedAtCache;
}

function catalogFreshnessDate(ctx, sourcePath) {
  if (!CATALOG_FRESHNESS_SOURCE_PATHS.has(sourcePath)) return "";
  return resolvedCatalogUpdatedAt(ctx);
}
...
function articleDatesFor(ctx, gitLastmodResolver = gitLastmod) {
  ...
  const dateModified = [
    ...
    gitDate,
    datePublished,
    catalogFreshnessDate(ctx, sourcePath),
  ]
    .filter(Boolean)
...
function breadcrumbEntity(ctx, canonical, pageTitle) {
  ...
  const requestedParent = typeof ctx.breadcrumbParent === "string"
    ? ctx.breadcrumbParent.trim()
    : "";
  const parent = requestedParent && requestedParent !== ctx.url
  ...
  if (parent) {
    crumbs.push({ name: parent.title || parent.linkTitle, item: absURL(parent.url) });
...
//        noindexFollow,
//        cve, canonicalCveRoute, date, lastmod, disclosed, howTo,
//        breadcrumbParent, sourcePath,
