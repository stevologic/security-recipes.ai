...
// Keep these in sync with scripts/cve_text_quality.py and the browser catalog
// cleaners. Supplementary-plane ranges use surrogate pairs so the public
// ES5-style scripts do not depend on Unicode code-point regex support.
const HAN_IDEOGRAPH_RE = /(?:[\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF]|[\uD840-\uD87D][\uDC00-\uDFFF]|\uD87E[\uDC00-\uDE1F]|[\uD880-\uD88C][\uDC00-\uDFFF]|\uD88D[\uDC00-\uDC7F])/;
const MIXED_CJK_RUN_RE = /(?:[\u2E80-\u312F\u3190-\u33FF\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF\uFF01-\uFF0F\uFF1A-\uFF20\uFF3B-\uFF40\uFF5B-\uFF9F]|[\uD840-\uD87D][\uDC00-\uDFFF]|\uD87E[\uDC00-\uDE1F]|[\uD880-\uD88C][\uDC00-\uDFFF]|\uD88D[\uDC00-\uDC7F])+/g;
const EMPTY_ASCII_PAIR_RE = /\(\s*\)|\[\s*\]|\{\s*\}/g;

function removeHanBearingCjkRuns(value) {
  if (!HAN_IDEOGRAPH_RE.test(value)) return value;
  let cleaned = value.replace(MIXED_CJK_RUN_RE, " ");
  for (;;) {
    const withoutEmptyPairs = cleaned.replace(EMPTY_ASCII_PAIR_RE, " ");
    if (withoutEmptyPairs === cleaned) return cleaned;
    cleaned = withoutEmptyPairs;
  }
}
...
function cleanCatalogText(value) {
  ...
  text = text
    ...
    .replace(/\uFFFDs\b/gu, "'s")
    .replace(/\uFFFD/gu, " ");
  return removeHanBearingCjkRuns(text)
    .replace(/\s+/gu, " ")
    .trim();
}
