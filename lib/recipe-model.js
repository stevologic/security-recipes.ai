...
const CVE_WORKFLOW_ROLES = new Set(["remediate", "contain", "audit", "intake"]);
...
function explicitStringList(value) {
  if (!Array.isArray(value)) return [];
  return [...new Set(value.map((item) => String(item || "").trim().toLowerCase()))]
    .filter(Boolean);
}

function explicitCveWorkflowRole(value, hasMappings) {
  const role = String(value || "").trim().toLowerCase();
  if (!role) return hasMappings ? "remediate" : "";
  if (!CVE_WORKFLOW_ROLES.has(role)) {
    throw new Error(`Unsupported cve_workflow_role: ${role}`);
  }
  return role;
}
...
function qualityFor(plainLower, facetCount) {
  ...
  if (has("## output contract", "## required output contract")) { signals.push("output-contract"); score += 15; }
...
function recipeModel(page) {
  ...
  const cveArchetypes = explicitStringList(fm.cve_archetypes);
  const cveWorkflowRole = explicitCveWorkflowRole(
    fm.cve_workflow_role,
    cveArchetypes.length > 0
  );
  ...
  return {
    ...
    aiAssisted: fm.ai_assisted === true,
