...
description: >
  ...
  audience validation, PKCE, RFC 9207 issuer mix-up checks, OAuth-metadata
  SSRF controls, client metadata, scope challenges, and step-up authorization.
sidebar:
  exclude: true
...
- Were protected-resource metadata, authorization-server metadata, and
  Client ID Metadata Document URLs HTTPS fetches that are not private,
  loopback, or link-local SSRF targets?
...
authorization decisions. Rechecked September 25, 2026: MCP
...
with simple string comparison before redeeming a code. Server-side MCP
clients **MUST** treat OAuth metadata discovery as SSRF-capable: require
HTTPS and **SHOULD** block private, loopback, and link-local fetch
targets as recommended by
[RFC 9728 Section 7.7](https://www.rfc-editor.org/rfc/rfc9728.html#section-7.7).
The same fetch restrictions apply to Client ID Metadata Documents.
Dynamic Client Registration is **deprecated**; Client ID Metadata
Documents remain the preferred registration method. `--session-id` and
`kill_session` here are local run identifiers and host-session kill
switches, not `Mcp-Session-Id`. Session binding in this pack means OAuth
and token-to-run binding. Streamable HTTP revisions through
[2025-11-25](https://modelcontextprotocol.io/specification/2025-11-25)
could assign that header; 2026-07-28 ignores it and does not mint
session IDs. This change does not claim independent human review of the
...
Reject a protected-resource metadata URL that points at the cloud
metadata service:
...
| `deny_oauth_metadata_ssrf` | A protected-resource, authorization-server, or client metadata URL is not HTTPS or targets a private, loopback, link-local, or cloud-metadata fetch destination. |
...
validation, redirect policy, OAuth-metadata SSRF checks, scope
...
- [RFC 9728 Section 7.7](https://www.rfc-editor.org/rfc/rfc9728.html#section-7.7)
  for blocking OAuth metadata fetches to internal IP address ranges.
...
- replay confused-deputy, issuer mix-up, unbound-token, and
  OAuth-metadata SSRF tests,
