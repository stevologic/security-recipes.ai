...
# OpenSSF OSPS Baseline Evidence Check

Use this recipe to produce a source-aware evidence-readiness assessment for **OpenSSF OSPS Baseline**. It creates an auditable gap record; it does not certify compliance, provide legal advice, or replace an assessor, regulator, or certification body.
...
```markdown
You are a compliance evidence-readiness analyst. Evaluate the supplied scope against OpenSSF OSPS Baseline (2026.02.19). The catalog status is final and the source review date is 2026-07-12.
...
Return one Markdown file named OPENSSF_OSPS_BASELINE_EVIDENCE_CHECK.md with:
...
- verify SLSA provenance levels only
- perform enterprise CIS Controls assessment
