...
scope: "platform"
ecosystem: "kubernetes/manifests"
facets: ["code-hygiene", "audit", "remediation"]
tags: ["code-hygiene", "platform", "kubernetes", "probes", "resources", "rollout"]
weight: 1506
date: 2026-07-12
reference_ids: ["nist-ssdf", "terraform-validate", "docker-build", "kubernetes-config", "github-actions-syntax", "kubernetes-probes"]
references_reviewed: 2026-07-12
---

<!-- Generated by scripts/sync_code_hygiene_recipes.py; edit data/code-hygiene/catalog.json. -->

A tool-agnostic recipe to make probes, resources, disruption, and rollout settings internally consistent. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to make probes, resources, disruption, and rollout settings internally consistent. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.platform.kubernetes-probe-resource-and-rollout-hygiene`.
...
Your bounded objective is to make probes, resources, disruption, and rollout settings internally consistent.
...
- Inspect startup, readiness, and liveness probes; requests and limits; termination grace; strategy; disruption budgets; and autoscaling inputs.
- Relate probe endpoints to actual dependency and startup behavior visible in code.
...
- Correct contradictory or clearly invalid configuration while leaving capacity and SLO choices to owners.
...
- Render and validate manifests and test probe endpoints and graceful shutdown locally where possible.
...
- Stop before choosing production resource values, availability budgets, or rollout percentages without operational evidence.
...
- [Terraform format, validation, and provider-lock hygiene]({{< relref "/prompt-library/general/code-hygiene/platform/terraform-fmt-validate-and-provider-lock" >}})
- [Terraform state-address and refactor safety]({{< relref "/prompt-library/general/code-hygiene/platform/terraform-state-address-and-refactor-safety" >}})
...
- [Kubernetes liveness, readiness, and startup probes](https://kubernetes.io/docs/concepts/configuration/liveness-readiness-startup-probes/) — Kubernetes
