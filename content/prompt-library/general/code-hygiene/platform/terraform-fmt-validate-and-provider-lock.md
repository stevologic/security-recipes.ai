...
model: "gpt-5-codex"
recipe_id: "code-hygiene.platform.terraform-fmt-validate-and-provider-lock"
...
A tool-agnostic recipe to keep configuration canonical and provider resolution reproducible. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to keep configuration canonical and provider resolution reproducible. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.platform.terraform-fmt-validate-and-provider-lock`.
...
Your bounded objective is to keep configuration canonical and provider resolution reproducible.
...
- Inventory Terraform versions, required providers, lock hashes, modules, generated configuration, and workspace boundaries.
- Run formatting and validation without accessing remote backends where possible.
...
- Apply canonical formatting and regenerate lock metadata only with the declared toolchain and intended platforms.
...
- Run fmt check, init with backend disabled where suitable, validate, and a reviewed plan for affected roots.
...
- Stop on provider upgrades, backend access, lock hash provenance uncertainty, or state mutation.
...
- [Terraform dependency lock file](https://developer.hashicorp.com/terraform/language/files/dependency-lock) — HashiCorp
