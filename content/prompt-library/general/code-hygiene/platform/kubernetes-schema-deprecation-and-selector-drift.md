...
tags: ["code-hygiene", "platform", "kubernetes", "schema", "deprecation", "selectors"]
weight: 1505
date: 2026-07-12
...
A tool-agnostic recipe to remove deprecated fields and prevent selector and label contract drift. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to remove deprecated fields and prevent selector and label contract drift. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.platform.kubernetes-schema-deprecation-and-selector-drift`.
...
Your bounded objective is to remove deprecated fields and prevent selector and label contract drift.
...
- Render manifests and inspect apiVersion, schema validation, immutable selectors, labels, patches, CRDs, and target cluster versions.
- Compare base and overlay outputs rather than editing templates in isolation.
...
- Use supported APIs and align labels and selectors without changing resource identity unintentionally.
...
- Render every overlay and run server-side or version-matched validation without applying resources.
...
- Stop if migration changes immutable fields, CRDs, resource identity, or requires cluster access.
