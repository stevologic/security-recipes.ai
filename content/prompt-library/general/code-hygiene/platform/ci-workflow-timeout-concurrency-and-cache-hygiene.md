...
model: "gpt-5-codex"
recipe_id: "code-hygiene.platform.ci-workflow-timeout-concurrency-and-cache-hygiene"
recipe_kind: "code-hygiene"
scope: "platform"
ecosystem: "ci/workflows"
facets: ["code-hygiene", "audit", "remediation"]
tags: ["code-hygiene", "platform", "ci", "workflows", "timeouts", "concurrency", "cache"]
weight: 1507
date: 2026-07-12
...
A tool-agnostic recipe to bound CI jobs and prevent stale caches and duplicate workflow races. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to bound CI jobs and prevent stale caches and duplicate workflow races. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.platform.ci-workflow-timeout-concurrency-and-cache-hygiene`.
...
Your bounded objective is to bound CI jobs and prevent stale caches and duplicate workflow races.
...
- Map triggers, permissions, job dependencies, timeouts, concurrency groups, cancellation, matrices, cache keys, and artifact lifetimes.
- Identify redundant runs and caches missing lockfile, platform, or toolchain inputs.
...
- Add bounded timeouts, safe concurrency cancellation, and deterministic cache keys without changing release authority.
...
- Validate workflow syntax and exercise pull-request, branch, retry, and cache-hit or miss paths where tooling permits.
...
- Stop before changing secrets, permissions, release triggers, protected environments, or third-party action trust.
