...
tags: ["code-hygiene", "dart-flutter", "dart", "analyzer", "null-safety", "ignores"]
weight: 1201
date: 2026-07-12
...
A tool-agnostic recipe to resolve analyzer diagnostics and remove unsafe null assertions and broad ignores. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to resolve analyzer diagnostics and remove unsafe null assertions and broad ignores. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.dart-flutter.dart-analyzer-null-safety-and-ignore-debt`.
...
Your bounded objective is to resolve analyzer diagnostics and remove unsafe null assertions and broad ignores.
...
- Read analysis_options, SDK constraints, generated exclusions, ignore comments, dynamic values, casts, and bang assertions.
- Separate plugin and platform-channel boundaries from application-owned types.
...
- Use flow analysis, typed models, and narrow boundary validation without weakening analyzer rules.
...
- Run dart format check, analyze, and focused tests under the constrained SDK.
...
- Stop if the fix raises the SDK floor or changes a published package API.
...
- [Flutter Future, stream, controller, and widget lifecycle]({{< relref "/prompt-library/general/code-hygiene/dart-flutter/flutter-future-stream-controller-and-widget-lifecycle" >}})
