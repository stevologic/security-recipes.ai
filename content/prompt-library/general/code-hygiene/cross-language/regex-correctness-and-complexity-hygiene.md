...
tags: ["code-hygiene", "cross-language", "regex", "complexity", "parsing"]
weight: 114
date: 2026-07-12
...
A tool-agnostic recipe to make complex regular expressions bounded, readable, and behaviorally tested. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to make complex regular expressions bounded, readable, and behaviorally tested. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.cross-language.regex-correctness-and-complexity-hygiene`.
...
Your bounded objective is to make complex regular expressions bounded, readable, and behaviorally tested.
...
- Find nested quantifiers, ambiguous alternation, catastrophic backtracking risk, and undocumented capture dependencies.
- Collect representative valid, invalid, adversarial, and maximum-length inputs.
...
- Simplify or replace risky expressions while preserving explicit matching and capture contracts.
...
- Run positive, negative, boundary, Unicode, and adversarial performance tests.
...
- Stop if the expression implements an undocumented protocol or security boundary requiring specialist review.
