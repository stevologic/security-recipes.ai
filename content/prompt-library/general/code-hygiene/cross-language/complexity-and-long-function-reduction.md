...
tags: ["code-hygiene", "cross-language", "complexity", "control-flow", "refactor"]
weight: 104
date: 2026-07-12
...
A tool-agnostic recipe to reduce hard-to-review control flow without changing behavior. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to reduce hard-to-review control flow without changing behavior. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.cross-language.complexity-and-long-function-reduction`.
...
Your bounded objective is to reduce hard-to-review control flow without changing behavior.
...
- Use configured complexity diagnostics and inspect nesting, early exits, state mutation, and mixed responsibilities.
- Capture observable behavior before choosing extraction boundaries.
...
- Extract cohesive pure or side-effect-bounded units and simplify control flow in reviewable increments.
...
- Run characterization tests and compare outputs, exceptions, side effects, and performance-sensitive paths.
...
- Stop if behavior is untested, timing-dependent, protocol-sensitive, or requires a public API redesign.
