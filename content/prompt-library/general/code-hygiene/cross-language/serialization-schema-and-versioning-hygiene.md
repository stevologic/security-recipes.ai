...
tags: ["code-hygiene", "cross-language", "serialization", "schema", "versioning"]
weight: 113
date: 2026-07-12
...
A tool-agnostic recipe to make serialized contracts explicit and backward-compatible. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to make serialized contracts explicit and backward-compatible. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.cross-language.serialization-schema-and-versioning-hygiene`.
...
Your bounded objective is to make serialized contracts explicit and backward-compatible.
...
- Inventory encoders, decoders, schema files, field aliases, defaults, and persisted payload versions.
- Identify implicit field renames, lossy coercion, unknown-field behavior, and unstable ordering assumptions.
...
- Add explicit version handling and compatibility-preserving field behavior at the serialization boundary.
...
- Round-trip current fixtures and read representative prior-version payloads.
...
- Stop if compatibility requirements or migration ownership are unknown.
