...
model: "gpt-5-codex"
recipe_id: "code-hygiene.cross-language.test-isolation-and-fixture-hygiene"
recipe_kind: "code-hygiene"
scope: "cross-language"
ecosystem: "multi-language"
facets: ["code-hygiene", "audit", "remediation"]
tags: ["code-hygiene", "cross-language", "tests", "fixtures", "isolation"]
weight: 106
date: 2026-07-12
...
A tool-agnostic recipe to eliminate order-dependent fixtures and leaked test state. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to eliminate order-dependent fixtures and leaked test state. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.cross-language.test-isolation-and-fixture-hygiene`.
...
Your bounded objective is to eliminate order-dependent fixtures and leaked test state.
...
- Run tests alone and in varied order while tracking global, database, filesystem, environment, and mock state.
- Identify fixtures whose setup or cleanup is implicit, shared, or broader than the test.
...
- Make ownership explicit, scope fixtures narrowly, and restore every mutated boundary deterministically.
...
- Run randomized-order, parallel where supported, focused, and full-suite checks.
...
- Stop before deleting persistent fixtures or touching shared external environments without operator approval.
