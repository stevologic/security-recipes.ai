...
A tool-agnostic recipe to prevent stream stalls, duplicate listeners, and handles that keep processes alive. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to prevent stream stalls, duplicate listeners, and handles that keep processes alive. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.javascript-typescript.node-stream-emitter-and-handle-lifecycle`.
...
Your bounded objective is to prevent stream stalls, duplicate listeners, and handles that keep processes alive.
...
- Trace stream backpressure, pipeline errors, listener counts, timers, sockets, and handle close ownership.
- Find manual pipe chains and emitter listeners without terminal cleanup.
...
- Use lifecycle-aware stream composition and close, unref, or unsubscribe handles at the owning boundary.
...
- Test partial reads, errors, aborts, slow consumers, repeated startup, and clean process termination.
...
- Stop if changing buffering or event ordering could violate a protocol or throughput requirement.
...
- [Node.js streams API](https://nodejs.org/api/stream.html) — OpenJS Foundation
