...
tags: ["code-hygiene", "jvm", "java", "executor", "future", "threadlocal"]
weight: 404
date: 2026-07-12
reference_ids: ["nist-ssdf", "java-javac", "kotlin-conventions", "java-concurrency"]
references_reviewed: 2026-07-12
---

<!-- Generated by scripts/sync_code_hygiene_recipes.py; edit data/code-hygiene/catalog.json. -->

A tool-agnostic recipe to bound background work and clean executor and thread-local state. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to bound background work and clean executor and thread-local state. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.jvm.java-executor-future-and-threadlocal-lifecycle`.
...
Your bounded objective is to bound background work and clean executor and thread-local state.
...
- Trace executor creation, queue bounds, Future observation, cancellation, shutdown, and ThreadLocal removal.
- Find common-pool blocking and per-request state crossing reused threads.
...
- Assign explicit ownership, propagate cancellation, bound queues, observe failures, and clear thread-local state.
...
- Test saturation, cancellation, task failure, shutdown, and request reuse.
...
- Stop if changing concurrency limits requires capacity or latency decisions from an owner.
...
- [Java concurrency utilities](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/concurrent/package-summary.html) — Oracle
