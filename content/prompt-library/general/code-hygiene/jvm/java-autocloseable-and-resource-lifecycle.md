...
tags: ["code-hygiene", "jvm", "java", "autocloseable", "resources", "try-with-resources"]
weight: 403
date: 2026-07-12
reference_ids: ["nist-ssdf", "java-javac", "kotlin-conventions", "java-autocloseable"]
references_reviewed: 2026-07-12
---

<!-- Generated by scripts/sync_code_hygiene_recipes.py; edit data/code-hygiene/catalog.json. -->

A tool-agnostic recipe to close files, streams, clients, cursors, and scopes on every exit path. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to close files, streams, clients, cursors, and scopes on every exit path. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.jvm.java-autocloseable-and-resource-lifecycle`.
...
Your bounded objective is to close files, streams, clients, cursors, and scopes on every exit path.
...
- Find AutoCloseable creation and trace ownership through returns, exceptions, wrappers, and asynchronous work.
- Identify close ordering, suppressed exceptions, and resources escaping their intended scope.
...
- Use try-with-resources or an explicit owner whose lifetime matches the escaped resource.
...
- Test normal, partial-init, exception, and early-return paths while asserting closure order.
...
- Stop if ownership is shared or framework-managed and its lifecycle contract is unclear.
...
- [Java compiler warning, deprecation, and suppression debt]({{< relref "/prompt-library/general/code-hygiene/jvm/java-compiler-warning-deprecation-and-suppression-debt" >}})
- [Java nullability, Optional, and boundary contracts]({{< relref "/prompt-library/general/code-hygiene/jvm/java-nullability-optional-and-boundary-contracts" >}})
...
- [Java AutoCloseable API](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/AutoCloseable.html) — Oracle
