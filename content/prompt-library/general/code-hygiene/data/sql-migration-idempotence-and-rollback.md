...
model: "gpt-5-codex"
recipe_id: "code-hygiene.data.sql-migration-idempotence-and-rollback"
recipe_kind: "code-hygiene"
scope: "data"
ecosystem: "sql/database"
facets: ["code-hygiene", "audit", "remediation"]
tags: ["code-hygiene", "data", "sql", "migrations", "rollback", "schema"]
weight: 1401
date: 2026-07-12
...
A tool-agnostic recipe to make schema migrations ordered, restartable where required, and operationally reversible. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to make schema migrations ordered, restartable where required, and operationally reversible. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.data.sql-migration-idempotence-and-rollback`.
...
Your bounded objective is to make schema migrations ordered, restartable where required, and operationally reversible.
...
- Inspect migration framework ordering, transactional DDL support, backfills, locks, defaults, and deploy sequencing.
- Classify expand, migrate, contract, irreversible, and data-loss operations.
...
- Split incompatible changes into bounded forward-safe steps and document honest rollback or roll-forward behavior.
...
- Apply from a prior schema, retry interrupted steps where supported, and validate application compatibility before and after.
...
- Stop on destructive changes, unknown table scale, or lock duration requiring database-owner approval.
...
- [SQL query plan, index, and N+1 hygiene]({{< relref "/prompt-library/general/code-hygiene/data/sql-query-plan-index-and-n-plus-one" >}})
