...
tags: ["code-hygiene", "c-cpp", "c", "cpp", "ownership", "raii"]
weight: 802
date: 2026-07-12
...
A tool-agnostic recipe to make memory, file, socket, lock, and handle ownership explicit. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to make memory, file, socket, lock, and handle ownership explicit. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.c-cpp.c-cpp-ownership-raii-and-resource-lifecycle`.
...
Your bounded objective is to make memory, file, socket, lock, and handle ownership explicit.
...
- Trace allocation, acquisition, transfer, release, error jumps, destructors, and raw owning pointers.
- Identify mismatched allocators, double release, leaks, and resources held beyond need.
...
- Use scoped cleanup or RAII owners and express transfers without changing non-owning views.
...
- Run focused failure-path tests and configured leak, address, or resource sanitizers.
...
- Stop when ownership crosses an undocumented C ABI or third-party callback boundary.
