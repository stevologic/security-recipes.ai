...
tags: ["code-hygiene", "c-cpp", "c", "cpp", "const", "span", "string-view", "lifetimes"]
weight: 805
date: 2026-07-12
...
A tool-agnostic recipe to make mutation and non-owning view lifetimes explicit. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to make mutation and non-owning view lifetimes explicit. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.c-cpp.c-cpp-const-span-view-and-lifetime-hygiene`.
...
Your bounded objective is to make mutation and non-owning view lifetimes explicit.
...
- Find raw pointer-length pairs, dangling views, temporaries bound to references, unnecessary mutation, and const casts.
- Trace container reallocation and owner lifetime past every span or view use.
...
- Use explicit owner and view types with const-correct interfaces and lifetime-safe call structure.
...
- Run tests under sanitizers and exercise empty, temporary, reallocated, and moved-owner cases.
...
- Stop if interface changes affect ABI or callers outside the repository.
