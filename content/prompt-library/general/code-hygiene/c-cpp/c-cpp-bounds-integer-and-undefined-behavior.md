...
A tool-agnostic recipe to remove unchecked bounds, lossy arithmetic, lifetime, and undefined-behavior hazards. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to remove unchecked bounds, lossy arithmetic, lifetime, and undefined-behavior hazards. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.c-cpp.c-cpp-bounds-integer-and-undefined-behavior`.
...
Your bounded objective is to remove unchecked bounds, lossy arithmetic, lifetime, and undefined-behavior hazards.
...
- Trace lengths, offsets, signedness, narrowing, pointer arithmetic, shifts, overflow assumptions, and object lifetimes.
- Exercise parser and allocation boundaries with sanitizer-supported inputs.
...
- Validate before arithmetic, use suitable types and checked operations, and preserve object lifetime rules.
...
- Run boundary tests, fuzz targets if present, and address and undefined-behavior sanitizers.
...
- Stop if wire format, ABI width, or performance constraints make the safe representation unclear.
