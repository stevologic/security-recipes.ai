...
A tool-agnostic recipe to remove shared mutable defaults and ambiguous sentinel values. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to remove shared mutable defaults and ambiguous sentinel values. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.python.python-mutable-default-dataclass-and-sentinel-hygiene`.
...
Your bounded objective is to remove shared mutable defaults and ambiguous sentinel values.
...
- Find mutable parameter and field defaults, reused class state, and None overloaded as both data and absence.
- Inspect dataclass equality, ordering, frozen, slots, and default_factory behavior.
...
- Use factories or private sentinels while preserving signatures, serialization, and equality contracts.
...
- Test repeated construction, mutation isolation, omitted versus explicit null values, and serialization.
...
- Stop if sentinel changes affect a public API or persisted representation without compatibility handling.
...
- [Python exception, context, and resource lifecycle]({{< relref "/prompt-library/general/code-hygiene/python/python-exception-chaining-context-and-resource-lifecycle" >}})
...
- [Python dataclasses](https://docs.python.org/3/library/dataclasses.html) — Python Software Foundation
