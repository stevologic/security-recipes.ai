...
model: "gpt-5-codex"
recipe_id: "code-hygiene.python.python-asyncio-task-cancellation-and-timeouts"
...
A tool-agnostic recipe to eliminate orphaned tasks, swallowed cancellation, and unbounded awaits. It supports a read-only audit and, when explicitly authorized, a narrow remediation.

## When to use it

Use this recipe when the operator's specific objective is to eliminate orphaned tasks, swallowed cancellation, and unbounded awaits. Prefer a narrower CVE, scanner-finding, security-audit, or compliance-evidence recipe when that is the operator's actual job.
...
~~~markdown
You are running the Security Recipes code-hygiene workflow `code-hygiene.python.python-asyncio-task-cancellation-and-timeouts`.
...
Your bounded objective is to eliminate orphaned tasks, swallowed cancellation, and unbounded awaits.
...
- Trace create_task ownership, TaskGroup use, cancellation propagation, timeout boundaries, and blocking calls.
- Find background tasks held only weakly or exceptions never observed.
...
- Use structured task ownership, explicit timeout boundaries, and finally cleanup while re-propagating cancellation.
...
- Test cancellation at each await, timeout expiry, child failure, shutdown, and task completion.
...
- Stop if task lifetime is intentionally process-global but has no documented owner.
