...
sidebar:
  open: true
description: >
  Deterministic, well-worn automation for risk reduction â€” Dependabot,
...
flows earn their keep on the problems these tools can't touch â€”
...
- **[GitHub Dependabot](https://docs.github.com/en/code-security/dependabot)** â€” version updates, security updates, grouped
...
- **[Renovate](https://docs.renovatebot.com/)** â€” the open-source competitor to Dependabot with
...
- **npm audit / pnpm audit / yarn audit** â€” built-in CVE reports.
  ...
  carefully).
- **[pip-audit](https://github.com/pypa/pip-audit)** â€” `pip-audit --fix` rewrites `requirements.txt`
  pinning against PyPI's vulnerability database. Pair with a
  `requirements.in` / `pip-compile` workflow for clean diffs.
- **uv / Poetry** â€” both expose `add`/`lock` commands that resolve
  bumps deterministically; Poetry has a `poetry update` and uv has
  `uv lock --upgrade-package`.
- **Go** â€” `go get -u ./...` followed by `go mod tidy` gives you a
  clean, verifiable bump. `govulncheck` reports reachable CVEs.
- **Cargo** â€” `cargo update` for bumps; `cargo audit` for CVE
  reporting via RustSec.
- **Bundler** â€” `bundle update --conservative <gem>` bumps a single
  gem without cascading the rest of the lockfile.
- **[Mend](https://www.mend.io/)** / **[Black Duck](https://www.blackduck.com/)** / **[JFrog Xray](https://jfrog.com/xray/)** â€” enterprise SCA platforms with broad ecosystem coverage, policy workflows, and license compliance.
- **[OWASP Dependency-Check](https://owasp.org/www-project-dependency-check/)** / **[OSV-Scanner](https://google.github.io/osv-scanner/)** â€” free/open-source SCA options that fit well in CI for teams that want self-managed scanning.

### Code scanning & lint auto-fix

- **[GitHub code scanning (CodeQL)](https://docs.github.com/en/code-security/code-scanning/introduction-to-code-scanning/about-code-scanning-with-codeql)** â€” SARIF output, PR annotations,
  default setup for most languages. Starts showing value the same day
  you flip it on.
- **ESLint / Prettier / Biome** â€” `--fix` flags. Wire into a
  pre-commit hook and a CI job; most style drift disappears on its
  own.
- **[Ruff](https://docs.astral.sh/ruff/)** â€” `ruff check --fix` and `ruff format` for Python. Fast
  enough that there's no reason not to run it on every save.
- **[gofmt](https://pkg.go.dev/cmd/gofmt) / [goimports](https://pkg.go.dev/golang.org/x/tools/cmd/goimports)** â€” the table stakes for Go style.
- **[Clippy](https://doc.rust-lang.org/clippy/)** â€” `cargo clippy --fix` for Rust.
- **[Semgrep](https://semgrep.dev/)** â€” fast OSS + enterprise SAST with managed rule packs, custom rules, and autofix support for many findings.
- **[SonarQube](https://www.sonarsource.com/products/sonarqube/) / [SonarCloud](https://www.sonarsource.com/products/sonarcloud/)** â€” widely adopted code quality + security gates for pull requests and release branches.

### Secret detection

- **[Gitleaks](https://github.com/gitleaks/gitleaks)** â€” open-source SAST for secrets. Detects hardcoded
  ...
  Typical deployment:
  - **Pre-commit hook** â€” `gitleaks protect --staged` blocks a
    commit before the secret ever lands in history.
  - **CI job** â€” `gitleaks detect --source . --log-opts="--all"`
    scans the full history on every PR and fails the build if a
    new secret appears.
  - **GitHub Action** â€” the official `gitleaks/gitleaks-action` is
...
- **[TruffleHog](https://github.com/trufflesecurity/trufflehog)** â€” credential detection with **verified / unverified**
  ...
  access.
- **[GitHub secret scanning](https://docs.github.com/en/code-security/secret-scanning/introduction/about-secret-scanning)** â€” built into GitHub for supported
  partners, with push protection. Flip it on â€” zero config, and
...
broader category â€” PII, PHI, PCI data, and other regulated
...
- **Earlybird** (American Express,
  ...
  â€” open-source SDE scanner with a deliberately broad module set:
...
- **[OPA](https://www.openpolicyagent.org/) / [Conftest](https://www.conftest.dev/)** â€” deterministic policy checks against Terraform
  plans, Kubernetes manifests, Dockerfiles. The fix is usually "edit
  the file"; the automation is the *enforcement* of what "correct"
  means.
- **[tfsec](https://aquasecurity.github.io/tfsec/) / [Checkov](https://www.checkov.io/) / [kube-linter](https://github.com/stackrox/kube-linter)** â€” category-specific scanners
  that ship with sensible defaults and `--fix` in many cases.
- **[Spectral](https://github.com/stoplightio/spectral) / [42Crunch API Security Testing](https://42crunch.com/)** â€” API contract linting and security checks for OpenAPI specs in CI.

### Container image scanning

- **[Trivy](https://trivy.dev/latest/)** (Aqua) â€” fast, open-source scanner for OS packages,
  language dependencies, IaC, and secrets inside container
  images. `trivy image <image>` on every build is the default
  starting point.
- **[Grype](https://github.com/anchore/grype)** (Anchore) â€” companion to Syft (SBOM) and widely used
  in CI. Integrates cleanly with Sigstore and Cosign for signed
  SBOM attestations.
- **[Clair](https://github.com/quay/clair)** (Quay) â€” registry-side scanning; pairs well with
  Harbor / Quay-style private registries.
- **[Docker Scout](https://docs.docker.com/scout/)** â€” Docker Hubâ€“native scanning with
  vulnerability policy and base-image recommendations.
- **[Snyk Container](https://docs.snyk.io/scan-with-snyk/snyk-container) / [Wiz](https://www.wiz.io/) / [Prisma Cloud](https://www.paloaltonetworks.com/prisma/cloud) / [Aqua](https://www.aquasec.com/)** â€” vendor
...
- **Minimal by design** â€” distroless, scratch-plus-libc, or a
...
- **Rebuilt on a known cadence** â€” nightly, weekly, or on any
  upstream CVE landing in the base packages â€” so fixes flow
  downstream automatically.
- **Versioned and lifecycled** â€” `myorg/base:python-3.12-2026.04`
...
- **Application-team triage collapses to a yes/no question.**
  ...
  Application teams don't re-triage base-layer CVEs â€” they just
  rebuild.
- **Scan reports become actionable.** Findings that are *not*
  in the golden image are findings the application team
  introduced â€” which is the small, focused fix set they can
  actually own.
...
myorg/base:python-3.12-2026.04 â†’ 2026.05` bump, fully
...
across a layered Dockerfile â€” much more blast radius, much
harder to gate safely.

**Representative tooling:**

- **Chainguard Images** â€” distroless, minimal-CVE, signed,
  SBOM-embedded base images rebuilt continuously. Vendor
  with a free tier.
- **Wolfi** (Chainguard) â€” open-source distro purpose-built for
  container images; widely used as the foundation for custom
  golden images.
- **Google Distroless** â€” open-source minimal images for the
  common language runtimes. The original distroless lineage.
- **Red Hat Universal Base Images (UBI)** â€” enterprise-supported
  base images with a stable CVE-remediation SLA.
- **Microsoft CBL-Mariner / Azure Linux** â€” minimal-footprint
  distribution used as a base in Azure and CI environments.
- **Sigstore Cosign + Syft** â€” open-source signing + SBOM
...
- **GitHub Actions** â€” a tiny workflow that runs `npm audit fix` /
...
- **Scheduled `make fix`** â€” if you have a `Makefile` target that
...
7. **Only then** layer the agent recipes on top â€” they'll have much
   less to do, and the work that's left is genuinely the work that
   needs judgment.

## See also

- [Agents]({{< relref "/agents" >}}) â€” per-tool remediation recipes for the problems automation can't handle
- [MCP Integration]({{< relref "/mcp-servers" >}}) â€” how agents reach the context that deterministic tools don't need
- [Agentic Security Remediation]({{< relref "/security-remediation" >}}) â€” security-team-operated workflows that combine both patterns
- [Recipes]({{< relref "/prompt-library" >}}) â€” community prompts that extend automation into judgment calls
